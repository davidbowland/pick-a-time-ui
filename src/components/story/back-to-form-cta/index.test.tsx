...
describe('BackToFormCta', () => {
  ...
  it('is hidden while the hero section is in view', () => {
    jest.mocked(useIsIntersecting).mockImplementation((ref) => ref === heroRef)
    render(<BackToFormCta footerRef={footerRef} formRef={formRef} heroRef={heroRef} onJump={jest.fn()} />)
  ...
  it('is visible when the hero, form, and footer are all out of view', () => {
  ...
  it('calls onJump when clicked', async () => {
    ...
    render(<BackToFormCta footerRef={footerRef} formRef={formRef} heroRef={heroRef} onJump={onJump} />)

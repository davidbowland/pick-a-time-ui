...
/**
 * Official Google "G" logo extracted from Google's pre-approved Sign-in
 * branding assets (the light-themed pill SVG served by
 * developers.google.com/identity/branding-guidelines).
 *
 * Wrapped in a white circle so the logo stays TOS-compliant on any
 * background color, including dark mode.
 *
 * @see https://developers.google.com/identity/branding-guidelines
 */
export const GoogleLogo = ({ size = 18 }: { size?: number }): React.ReactNode => (
  <span
    aria-hidden="true"
    className="inline-flex shrink-0 items-center justify-center rounded-full"
    style={{ width: size, height: size, backgroundColor: '#FFFFFF' }}
  >
    <svg fill="none" height={size * 0.78} viewBox="0 0 20 20" width={size * 0.78} xmlns="http://www.w3.org/2000/svg">
      {/* Paths below are from Google's official branding asset (clip0_710_6229) */}
      <path
